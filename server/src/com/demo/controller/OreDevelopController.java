...
@CrossOrigin
public class OreDevelopController  extends Controller {
	
	public static final String DB_TABLE = "ore_develop";
	...
	@CrossOrigin
	public void getAll(){
		List<OreDevelopModel> models = OreDevelopModel.dao.find("select * from "+DB_TABLE+" where del != 'delete'");
	...
	public void get(){
		...
		List<OreDevelopModel> models = OreDevelopModel.dao.find("select * from "+DB_TABLE+" where id = "+id+" and  del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null&&models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
	...
	public void add(){
		JSONObject js = new JSONObject();
		try{
			OreDevelopModel model = getModel(OreDevelopModel.class, "", true);
	...
	public void search(){
		...
		List<OreDevelopModel> models = OreDevelopModel.dao.find("select * from "+DB_TABLE+" where "+map+" like '%"+key+"%' and del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null && models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
			js.put(Const.KEY_RES_DATA, models);
