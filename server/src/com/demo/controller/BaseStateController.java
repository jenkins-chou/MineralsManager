...
@CrossOrigin
public class BaseStateController  extends Controller {
	
	public static final String DB_TABLE = "base_state";
	...
	@CrossOrigin
	public void getAll(){
		List<BaseStateModel> models = BaseStateModel.dao.find("select * from "+DB_TABLE+" where del != 'delete'");
	...
	public void get(){
		...
		List<BaseStateModel> models = BaseStateModel.dao.find("select * from "+DB_TABLE+" where id = "+id+" and  del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null&&models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
	...
	public void add(){
		JSONObject js = new JSONObject();
		try{
			BaseStateModel model = getModel(BaseStateModel.class, "", true);
