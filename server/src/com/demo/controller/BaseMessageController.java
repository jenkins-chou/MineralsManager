...
@CrossOrigin
public class BaseMessageController  extends Controller {
	
	public static final String DB_TABLE = "base_message";
	...
	@CrossOrigin
	public void getAll(){
		List<BaseMessageModel> models = BaseMessageModel.dao.find("select * from "+DB_TABLE+" where del != 'delete'");
	...
	public void get(){
		...
		List<BaseMessageModel> models = BaseMessageModel.dao.find("select * from "+DB_TABLE+" where id = "+id+" and  del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null&&models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
	...
	public void add(){
		JSONObject js = new JSONObject();
		try{
			BaseMessageModel model = getModel(BaseMessageModel.class, "", true);
